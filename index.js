...
async function updateTimer() {
    try {
        ...
        if (messageId) {
            try { const msg = await channel.messages.fetch(messageId); await msg.edit({ embeds: [embed] }); }
            catch { const m = await channel.send({ embeds: [embed] }); messageId = m.id; }
        } else { const m = await channel.send({ embeds: [embed] }); messageId = m.id; }
